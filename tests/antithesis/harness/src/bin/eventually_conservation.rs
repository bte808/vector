...
async fn main() {
    ...
    // fault injection across every container and kills the producers and the reload
    // helper; an `anytime_` already running finishes and nothing new starts. So for the
    // rest of this program the cluster is load-free and fault-free: no partitions, drops,
    // or latency faults to tolerate, only recovery to wait out. That is why the checks
    // below assert unconditionally — a shortfall now is real loss, and a probe that never
    // round-trips is a real wedge, not a transient fault.
